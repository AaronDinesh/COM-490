...
#
# Since we will be using spark, it is recommended to keep the [pyspark python API documentation](https://spark.apache.org/docs/latest/api/python/reference/index.html) on the side for the exercises.
...
# %%
gutenberg_rdd = sc.sequenceFile('/data/com-490/gutenberg/rdd').setName('gutenberg_rdd').cache()
...
# %%
import json
meta = sc.textFile('/data/com-490/gutenberg/gutenberg_metadata.json').collect()[0]
...
def filter_func(gid) : 
    ...
    return title!=None and first_name!=None and last_name!=None and birth_year!=None and birth_year.find("BC")==-1 and (death_year==None or death_year.find("BC")==-1)
...
# 2. keep only the unique author strings (*hint*: look at the [Spark RDD API](https://spark.apache.org/docs/latest/api/python/reference/api/pyspark.RDD.html) to find an appropriate method)
...
author_rdd = filtered_rdd.map(lambda x: u"{0}, {1}".format(meta_b.value[x[0]]['last_name'], meta_b.value[x[0]]['first_name']))
...
author_rdd.map(lambda x : (x, 1)).reduceByKey(add).takeOrdered(20,key=lambda x:-x[1])
...
lang_rdd = filtered_rdd.map(lambda x : u"{0}".format(meta_b.value[x[0]]['language']))

# %%
# TODO: reduce the `lang_rdd` to yield the number of books in each language
(lang_rdd.map(lambda x : (x, 1)).reduceByKey(add).takeOrdered(20,key=lambda x:-x[1]))
...
# 4. map the entire RDD of text into an RDD of single words (you need to use [flatMap](https://spark.apache.org/docs/latest/api/python/reference/api/pyspark.RDD.flatMap.html) -- this returns a different number of elements than it takes in)
...
english_rdd = filtered_rdd.filter(lambda x: meta_b.value[x[0]]['language'] == 'en')

# %%
# TODO: define `distinct_rdd` which should contain all lowercase, unique words in the english books of the corpus
distinct_rdd = english_rdd.flatMap(lambda x : remove_punctuation(x[1]).lower().split())
...
stop_words = sc.binaryFiles('/data/com-490/gutenberg/stop_words.dump').values().map(lambda p: pickle.load(BytesIO(p)))
...
words_rdd = english_rdd.flatMap(lambda x : remove_punctuation(x[1]).lower().split()).filter(lambda x: not x in stop_words_b.value)
...
# Create a look-up table of words by attaching a unique index to each word. The Spark API provides a [zipWithIndex](https://spark.apache.org/docs/latest/api/python/reference/api/pyspark.RDD.zipWithIndex.html) method that makes this easy. 
#
...
# 3. finally, use [collectAsMap](https://spark.apache.org/docs/latest/api/python/reference/api/pyspark.RDD.collectAsMap.html) to return the resulting RDD to the driver as a dictionary. 
...
# %%
# TODO
vector_rdd = english_rdd.mapValues(lambda text: vectorize_doc(remove_punctuation(text).lower().split(), word_lookup_b.value)).cache()
